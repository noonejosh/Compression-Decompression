...
public:
    ...
    void compressFile(string inputFile) {
        ...
        system("cls");
        cout << "\n\n\t\t\t\tSuccessful: File has been compressed.\n \t\t\t\tYour compressed filename is compressed.huff." << endl;
        cout << "\t\t\t\t";
        system("pause");
        inputedFile.close();
    ...
    void retrieveTree() {
        ifstream inFile("huffmanCode.txt");

        if (!inFile.is_open()) {
            cout << "\t\t\t\tError: Unable to open file for tree retrieval." << endl;
            return;
        }
        else {
            int character;
            int frequency;
            while (inFile >> character >> frequency) {
                enqueue((char)character, frequency);
            }
        }
        inFile.close();
    }
